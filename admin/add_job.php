...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        try {
            ...
            $conn->commit();
            $success = 'Job and screening questions added successfully!';
            $_POST = [];
            $questions = [];
        } catch (PDOException $e) {
            $conn->rollBack();
            $error = 'Error adding job. Please try again.';
...
<head>
    ...
    <meta name="description" content="Admin panel to add new job postings">
...
<body>
    ...
    <div class="container">
        <div class="card">
            <h1 class="text-2xl font-bold text-gray-900 mb-6">Create a New Job Posting</h1>
            ...
            <form action="add_job.php" method="POST" class="space-y-6" id="jobForm">
                <!-- Job Details Section -->
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="form-group md:col-span-2">
                            ...
                            <input
                                ...
                                id="requirements"
                                name="requirements"
                                value="<?php echo isset($_POST['requirements']) ? htmlspecialchars($_POST['requirements'], ENT_QUOTES, 'UTF-8') : ''; ?>"
                            >
                        </div>
                    </div>
                </div>
                                        ...
                                        if ($category['category_id'] == $category_id) {
                                            ...
                                            if (!empty($category['offshore_salary'])) {
                                                echo '<option value="' . htmlspecialchars($category['offshore_salary'], ENT_QUOTES, 'UTF-8') . '" ' .
                                                    (isset($_POST['salary_package']) && $_POST['salary_package'] == $category['offshore_salary'] ? 'selected' : '') .
                ...
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <input
                            ...
                            value="<?php echo isset($_POST['reference']) ? htmlspecialchars($_POST['reference'], ENT_QUOTES, 'UTF-8') : ''; ?>"
                        >
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="location" name="location" class="form-select" required>
                            ...
                            <option value="Dubai" <?php echo (isset($_POST['location']) && $_POST['location'] == 'Dubai') ? 'selected' : ''; ?>>Dubai</option>
                            <option value="Lahore" <?php echo (isset($_POST['location']) && $_POST['location'] == 'Lahore') ? 'selected' : ''; ?>>Lahore</option>
                        </select>
                    </div>
                </div>
    ...
    <script>
        ...
        categorySelect.addEventListener('change', function () {
            ...
            const salaryDropdown = document.getElementById('salary_package');
            salaryDropdown.innerHTML = '<option value="">Select Salary Option</option>';
            if (onsite) {
            ...
            document.getElementById('salary_package').value = ''; // Reset salary
        });

        // Auto-select location and perks based on salary package
        const salarySelect = document.getElementById('salary_package');
        salarySelect.addEventListener('change', function () {
            const selectedSalary = this.value;
            ...
            if (selectedSalary === onsiteSalary && onsiteSalary) {
                ...
                perksTextarea.value = onsitePerks; // Show onsite perks
            } else if (selectedSalary === offshoreSalary && offshoreSalary) {
                locationSelect.value = 'Lahore';
                perksTextarea.value = perks; // Show offshore perks
            } else {
                locationSelect.value = '';
                perksTextarea.value = ''; // Clear perks if no valid salary
            }
        });

        // Trigger category and salary change on page load if pre-selected
        window.addEventListener('load', () => {
            if (categorySelect.value) {
                categorySelect.dispatchEvent(new Event('change'));
            }
            if (salarySelect.value) {
                salarySelect.dispatchEvent(new Event('change'));
            }
        });
